...
def load_data(directory: str) -> dict[str, Any]:
    ...
    for name in ("daily", "future", "status"):
        frame = frames[name]
        if "symbol" in frame:
...
def forecast_path(symbol: str, anchor_price: float) -> tuple[pd.DataFrame, bool]:
    ...
    raw_lower = pd.to_numeric(coin.get("prediction_lower_95_error_band"), errors="coerce")
    raw_upper = pd.to_numeric(coin.get("prediction_upper_95_error_band"), errors="coerce")
...
def decision_score(
    ...
    if not model_fresh and action.startswith("STRONG"):
...
page = st.sidebar.radio("Workspace", ["Live Terminal", "Signal Reactor", "Market Radar", "Easy Mode"])
...
model_mode = "CURRENT 30D MODEL" if model_fresh else "30D SCENARIO MODE"
