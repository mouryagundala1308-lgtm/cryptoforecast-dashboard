"""CryptoForecast market monitoring, scenario and model-validation dashboard."""
...
def original_performance_metrics(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep successful original-data groups shared by all three models."""
    ...
    valid = valid.dropna(subset=["model", "mae", "rmse", "mape"])
    ...
    common_groups = common_groups.loc[common_groups].reset_index()[["symbol", "horizon_days"]]
    if common_groups.empty:
        return valid.iloc[0:0].copy()
    return valid.merge(common_groups, on=["symbol", "horizon_days"], how="inner")


def original_model_summary(frame: pd.DataFrame, horizon: int | None = None) -> pd.DataFrame:
    """Summarise original-data performance for the selected forecast horizon."""

    valid = original_performance_metrics(frame)
    if horizon is not None:
        valid = valid.loc[pd.to_numeric(valid["horizon_days"], errors="coerce").eq(horizon)]
...
def find_uploaded_column(frame: pd.DataFrame, candidates: list[str]) -> str | None:
    """Find a CSV column using tolerant, case-insensitive label matching."""

    def normalized(value: Any) -> str:
        return "".join(character for character in str(value).strip().lower() if character.isalnum())

    available = {normalized(column): str(column) for column in frame.columns}
    for candidate in candidates:
...
def uploaded_numeric(series: pd.Series) -> pd.Series:
    """Convert ordinary numbers and formatted currency/percentage text safely."""
...
def prepare_uploaded_dataset(frame: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    """Standardise an uploaded market CSV for safe, read-only visual inspection."""

    timestamp_column = find_uploaded_column(
        frame, ["timestamp", "datetime", "date_time", "date", "time"]
    )
    price_column = find_uploaded_column(
        frame, ["price_usd", "close", "closing_price", "price", "last_price"]
    )
    symbol_column = find_uploaded_column(
        frame, ["symbol", "ticker", "asset_symbol", "coin_symbol", "asset"]
    )
    name_column = find_uploaded_column(frame, ["name", "coin_name", "asset_name"])
    volume_column = find_uploaded_column(
        frame, ["vol_24h", "volume", "total_volume", "total_vol", "quote_volume"]
    )

    errors: list[str] = []
    if timestamp_column is None:
        errors.append("A timestamp or date column was not detected.")
    if price_column is None:
        errors.append("A price_usd, close or price column was not detected.")
    if errors:
        return pd.DataFrame(), {
            "errors": errors,
    ...
    if pd.api.types.is_numeric_dtype(raw_timestamps):
        ...
        typical_timestamp = numeric_timestamps.dropna().abs().median()
        timestamp_unit = "ms" if finite(typical_timestamp) and typical_timestamp >= 1e11 else "s"
    ...
    prepared["timestamp"] = parsed_timestamps.dt.tz_convert(None)
    prepared["price"] = uploaded_numeric(frame[price_column])
    if symbol_column is not None:
        prepared["symbol"] = (
            frame[symbol_column].astype("string").str.strip().str.upper().replace("", pd.NA)
        )
    else:
        prepared["symbol"] = "UPLOADED ASSET"
    if name_column is not None:
        prepared["name"] = frame[name_column].astype("string").str.strip()
    if volume_column is not None:
        prepared["volume"] = uploaded_numeric(frame[volume_column])

    invalid_timestamp_rows = int(prepared["timestamp"].isna().sum())
    invalid_price_rows = int(prepared["price"].isna().sum())
    valid = prepared.dropna(subset=["timestamp", "price", "symbol"]).copy()
    valid = valid.loc[valid["price"].ge(0)].sort_values(["symbol", "timestamp"], kind="mergesort")
    duplicate_rows = int(valid.duplicated(["symbol", "timestamp"]).sum())
    if duplicate_rows:
        valid = valid.drop_duplicates(["symbol", "timestamp"], keep="last")
    ...
    if not path.empty:
        ...
        if not horizon_row.empty and path["open"].iloc[0] > 0:
            ...
            add_factor(
                ...
                (
                    f"{model_move:+.2f}% · strength {model_confidence:.0f}/100"
                    if finite(model_confidence)
                    else f"{model_move:+.2f}% · limited strength"
                ),
            )
...
def forecast_candles(path: pd.DataFrame, title: str, selected_day: int | None = None) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=path["date"], y=path["lower"], fill="tonexty", fillcolor="rgba(139,124,255,.10)", line=dict(width=0), name="Model uncertainty range"))
    fig.add_trace(go.Candlestick(
        x=path["date"], open=path["open"], high=path["high"], low=path["low"], close=path["close"],
        name="Model scenario candles", increasing_line_color=POSITIVE_COLOR, decreasing_line_color=NEGATIVE_COLOR,
...
st.sidebar.markdown('<div class="brand"><div class="brand-name">◈ Crypto<span>Forecast</span></div><div class="brand-sub">MARKET MONITORING · SCENARIOS · PERFORMANCE</div></div>', unsafe_allow_html=True)
page = st.sidebar.radio(
    "Workspace",
    ["Market Overview", "Live Market", "Decision Support", "Market Scanner", "Model Performance", "Data Lab"],
)
...
selected_forecast_rows = future.loc[
    future["symbol"].eq(symbol) & future["status"].eq("FORECAST_GENERATED")
].copy()
forecast_as_of = (
    pd.to_datetime(selected_forecast_rows["as_of_date"], errors="coerce").max()
    if not selected_forecast_rows.empty and "as_of_date" in selected_forecast_rows
    else pd.NaT
)
forecast_as_of_label = forecast_as_of.strftime("%d %b %Y") if pd.notna(forecast_as_of) else "date unavailable"
model_mode = (
    f"SCENARIO CURRENT · {forecast_as_of_label}"
    if model_fresh
    else f"SCENARIO STALE · {forecast_as_of_label}"
)
...
if not live_market.empty:
    ...
    st.sidebar.markdown('<span class="pill">STORED MARKET HISTORY</span>', unsafe_allow_html=True)
st.sidebar.markdown(f'<span class="pill pill-model">{html.escape(model_mode)}</span>', unsafe_allow_html=True)
st.sidebar.caption("Decision support only · No order execution")

ticker(live_market)

if page in {"Market Overview", "Decision Support"}:
    if model_fresh:
        st.success(
            f"Market prices are live and the selected model scenario is current as of {forecast_as_of_label}."
            if not live_market.empty
            else f"Stored market history is active. The selected model scenario is current as of {forecast_as_of_label}."
        )
    else:
        st.warning(
            f"Observed market prices may be current, but the model scenario is based on data through "
            f"{forecast_as_of_label}. It is shown as a low-weight planning scenario, not a new live forecast."
            if not live_market.empty
            else f"Both market history and the model scenario are stored. The scenario is based on data "
            f"through {forecast_as_of_label} and must not be interpreted as a current forecast."
        )
...
if page == "Live Market":
    ...
    st.markdown(f'<div class="terminal-head"><div class="live-line"><span class="live-dot"></span>OBSERVED MARKET · {html.escape(str(symbol).upper())} / USDT · {html.escape(timeframe.upper())}</div><div class="terminal-title">{html.escape(selected_label)}</div><div class="terminal-sub">Recorded market movement · scroll to zoom · {html.escape(str(interval_settings["detail"]))} detail inside the {html.escape(str(interval_settings["bar"]))} overview</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="metric-grid">' + metric("Observed price", money(anchor_price), "Latest available public spot", "") + metric(f"{bar_label} move", pct(interval_move), "Observed percentage change", "up" if finite(interval_move) and interval_move >= 0 else "down") + metric(f"{bar_label} change", ("+" if finite(interval_change) and interval_change >= 0 else "") + money(interval_change), "Observed absolute price change", "up" if finite(interval_change) and interval_change >= 0 else "down") + metric("Loaded-period move", pct(visible_change), history_mode, "up" if finite(visible_change) and visible_change >= 0 else "down") + '</div>', unsafe_allow_html=True)

    terminal = go.Figure()
    terminal.add_trace(go.Candlestick(x=history["timestamp"], open=history["open"], high=history["high"], low=history["low"], close=history["close"], name="Observed market candles", increasing_line_color=POSITIVE_COLOR, decreasing_line_color=NEGATIVE_COLOR, increasing_fillcolor="rgba(0,127,95,.42)", decreasing_fillcolor="rgba(201,47,72,.42)"))
    trend_period = 20 if len(history) >= 40 else max(3, min(10, len(history) // 2))
    if len(history) >= trend_period:
        terminal.add_trace(go.Scatter(x=history["timestamp"], y=history["close"].rolling(trend_period).mean(), name=f"Observed {trend_period}-candle trend", line=dict(color=ACCENT_COLOR, width=1.7)))
    terminal.update_layout(title=f"{str(symbol).upper()} observed market movement · {history_mode}", xaxis_rangeslider_visible=False, yaxis_title="USD", dragmode="zoom")
    ...
    if "Cached" in history_mode:
        st.caption(
            "Stored-history mode: Close values come from the project dataset; Open, High and Low are "
            "visual estimates derived from consecutive prices and are not exchange-observed OHLC values."
        )
    else:
        st.caption(
            "Observed-market mode: Open, High, Low, Close and volume are public Binance candle values. "
            "Nothing in this chart is a future-price prediction."
        )
    ...
    day = st.slider(
        ...
        30,
        7,
        help="Select which future model-scenario day should contribute to the market-watch assessment.",
    )
    ...
    st.markdown(f'<div class="terminal-head"><div class="live-line">DECISION SUPPORT · SCENARIO DAY {day}</div><div class="terminal-title">Evidence before action</div><div class="terminal-sub">Observed trend, momentum and volume · supported by a clearly labelled model scenario</div></div>', unsafe_allow_html=True)
    left, right = st.columns([.9, 1.25], gap="large")
    with left:
        st.markdown(f'<div class="signal-core"><div class="signal-content"><div class="signal-kicker">Market watch status</div><div class="signal-action {action_tone}">{html.escape(action)}</div><div class="signal-score">EVIDENCE RATING {score:+.0f} / 100</div></div></div>', unsafe_allow_html=True)
        freshness_label = "MODEL SCENARIO CURRENT" if model_fresh else "STALE MODEL SCENARIO · LOW WEIGHT"
        ...
        st.markdown('<div class="micro-grid">' + '<div class="micro"><div class="micro-label">Evidence strength</div><div class="micro-value">' + f'{decision["evidence_strength"]:.0f}/100' + '</div></div>' + '<div class="micro"><div class="micro-label">Scenario day target</div><div class="micro-value">' + html.escape(money(target_price)) + '</div></div>' + '<div class="micro"><div class="micro-label">Scenario move</div><div class="micro-value ' + ("up" if finite(target_move) and target_move >= 0 else "down") + '">' + html.escape(pct(target_move)) + '</div></div></div>', unsafe_allow_html=True)
    with right:
        st.plotly_chart(decision_gauge(decision), width="stretch", theme=None, config={"displayModeBar": False})
    st.caption(
        "The evidence rating is a normalized decision-support score, not the probability of profit. "
        "BUY WATCH and SELL WATCH identify conditions to monitor; they are not trade instructions."
    )
    ...
    with chart_left:
        if path.empty:
            ...
            st.plotly_chart(forecast_candles(path, f"Stored 30-day model scenario · selected day {day}", day), width="stretch", theme=None, config={"displaylogo": False})
    with chart_right:
        st.plotly_chart(contribution_chart(decision), width="stretch", theme=None, config={"displayModeBar": False})

    st.subheader(f"What-if calculator · {action.title()}")
    mode = st.segmented_control("Simulated action", ["BUY", "SELL"], default="BUY")
    ...
    st.caption(
        "What-if simulation only. It combines the latest available price with the selected stored scenario target. "
        "Fees are assumptions; slippage, spread, taxes, liquidity and exchange rules are not included."
    )

elif page == "Market Scanner":
    st.markdown('<div class="terminal-head"><div class="live-line"><span class="live-dot"></span>OBSERVED MARKET SCANNER</div><div class="terminal-title">Current movement and pressure</div><div class="terminal-sub">Public Binance spot changes and volume · no forecast values on this page</div></div>', unsafe_allow_html=True)
    if live_market.empty:
        ...
        st.caption(
            "Pressure alerts identify observed 24-hour moves of at least ±4%. "
            "They are volatility alerts, not predictions or Buy/Sell recommendations."
        )

elif page == "Model Performance":
    st.markdown(
        '<div class="terminal-head"><div class="live-line">ORIGINAL MODEL RESULTS</div>'
        '<div class="terminal-title">Naïve, ARIMA and XGBoost performance</div>'
        '<div class="terminal-sub">Backtest results produced from the original project dataset · lower error is better</div></div>',
        unsafe_allow_html=True,
    )

    original_valid = original_performance_metrics(comparison)
    available_horizons = sorted(
        pd.to_numeric(original_valid.get("horizon_days"), errors="coerce").dropna().astype(int).unique()
    )
    horizon_options = ["All available"] + [f"{horizon}D" for horizon in available_horizons]
    selected_horizon_label = st.segmented_control(
        "Performance horizon",
        horizon_options,
        default="All available",
        help="Choose a forecast horizon to compare all three models using the original backtest output.",
    )
    selected_horizon = (
        None if selected_horizon_label == "All available" else int(str(selected_horizon_label).removesuffix("D"))
    )
    performance_summary = original_model_summary(comparison, selected_horizon)
    display_names = {"Naive": "Naïve", "ARIMA": "ARIMA", "XGBoost": "XGBoost"}
    processed_assets = (
        int(status["status"].astype("string").str.upper().eq("PROCESSED").sum())
        if "status" in status else int(comparison["symbol"].nunique())
    )
    total_assets = int(status["symbol"].nunique()) if "symbol" in status else int(comparison["symbol"].nunique())
    horizon_note = "all eligible horizons" if selected_horizon is None else f"the {selected_horizon}-day horizon"
    st.caption(
        f"Original project data only · {processed_assets} of {total_assets} assets processed · "
        f"showing {horizon_note}."
    )

    if performance_summary.empty:
        st.warning("No eligible original-data results are available for the selected horizon.")
    else:
        cards = st.columns(3)
        for column, model in zip(cards, ["Naive", "ARIMA", "XGBoost"]):
            row = performance_summary.loc[performance_summary["model"].eq(model)]
            with column:
                if row.empty:
                    st.markdown(metric(display_names[model], "—", "No eligible result"), unsafe_allow_html=True)
                else:
                    result = row.iloc[0]
                    st.markdown(
                        metric(
                            display_names[model],
                            pct(result["mape"], signed=False),
                            f"Weighted MAPE · median MAE {money(result['median_mae'])} · median RMSE {money(result['median_rmse'])}",
                        ),
                        unsafe_allow_html=True,
                    )

        chart_data = performance_summary.copy()
        chart_data["Model"] = chart_data["model"].replace(display_names)
        metric_charts = st.columns(3)
        chart_specs = [
            ("MAPE", "mape", "Percentage error (%)", ".2f"),
            ("MAE", "median_mae", "Median absolute error (USD)", ".4f"),
            ("RMSE", "median_rmse", "Median squared-error penalty (USD)", ".4f"),
        ]
        for chart_column, (title, value_column, axis_title, text_format) in zip(metric_charts, chart_specs):
            figure = px.bar(
                chart_data,
                x="Model",
                y=value_column,
                ...
                category_orders={"Model": ["Naïve", "ARIMA", "XGBoost"]},
                color_discrete_sequence=[ACCENT_COLOR, SECONDARY_COLOR, "#8b98a8"],
                title=f"{title} · {horizon_note}",
            )
            figure.update_layout(showlegend=False, xaxis_title=None, yaxis_title=axis_title)
            with chart_column:
                st.plotly_chart(
                    style_chart(figure, 390),
                    width="stretch",
                    theme=None,
        ...
        for (horizon, model), group in original_valid.groupby(["horizon_days", "model"]):
            ...
            if usable.any():
                horizon_rows.append({
                    ...
                    "Model": display_names.get(str(model), str(model)),
                    "MAPE": float(np.average(values[usable], weights=weights[usable])),
                    "horizon_order": int(horizon),
                })
        horizon_frame = pd.DataFrame(horizon_rows)
        if not horizon_frame.empty:
            horizon_frame = horizon_frame.sort_values("horizon_order")
            horizon_figure = px.line(
                ...
                markers=True,
                category_orders={"Model": ["Naïve", "ARIMA", "XGBoost"]},
                title="Original backtest percentage error by forecast horizon",
            )
            horizon_figure.update_layout(
                xaxis_title="Forecast horizon",
                yaxis_title="Weighted MAPE (%) · lower is better",
            )
            st.plotly_chart(
                style_chart(horizon_figure, 440),
                width="stretch",
                theme=None,
                config={"displayModeBar": False},
            )

        performance_assets = sorted(original_valid["symbol"].dropna().astype(str).unique())
        if performance_assets:
            selected_performance_asset = st.selectbox(
                "Inspect one original-data asset",
                performance_assets,
                format_func=lambda value: str(value).upper(),
                key="original_performance_asset",
            )
            asset_performance = original_valid.loc[
                original_valid["symbol"].eq(selected_performance_asset)
            ].copy()
            asset_performance["Model"] = asset_performance["model"].replace(display_names)
            asset_performance["Horizon"] = (
                pd.to_numeric(asset_performance["horizon_days"], errors="coerce")
                .astype(int)
                .map(lambda value: f"{value}D")
            )
            asset_figure = px.line(
                asset_performance.sort_values("horizon_days"),
                x="Horizon",
                y="mape",
                color="Model",
                markers=True,
                category_orders={"Model": ["Naïve", "ARIMA", "XGBoost"]},
                title=f"{selected_performance_asset.upper()} original backtest error",
                hover_data={"mae": ":.4f", "rmse": ":.4f", "mape": ":.2f"},
            )
            asset_figure.update_layout(
                xaxis_title="Forecast horizon",
                yaxis_title="MAPE (%) · lower is better",
            )
            st.plotly_chart(
                style_chart(asset_figure, 420),
                width="stretch",
                theme=None,
                config={"displaylogo": False},
            )

        result_table = performance_summary[
            ["model", "median_mae", "median_rmse", "mape", "test_predictions", "asset_horizon_groups"]
        ].copy()
        result_table["model"] = result_table["model"].replace(display_names)
        result_table = result_table.rename(columns={
            "model": "Model",
            "median_mae": "Median MAE (USD)",
            "median_rmse": "Median RMSE (USD)",
            "mape": "Weighted MAPE (%)",
            "test_predictions": "Test predictions",
            "asset_horizon_groups": "Asset–horizon results",
        }).sort_values("Weighted MAPE (%)")
        ...
        st.dataframe(
            result_table.style.format({
                ...
                "Weighted MAPE (%)": "{:,.2f}",
                "Test predictions": "{:,.0f}",
                "Asset–horizon results": "{:,.0f}",
            }),
            width="stretch",
            hide_index=True,
        )
        st.caption(
            "MAE is the typical absolute USD error. RMSE gives more weight to large USD errors. "
            "MAPE expresses average absolute error as a percentage. Coin price scales differ, so MAE and RMSE "
            "are displayed as medians across eligible asset–horizon results; MAPE is weighted by test rows."
        )
        with st.expander("Models and evaluation method"):
            st.markdown(
                "**Naïve:** uses the latest known value as the benchmark forecast.  "
                "\n**ARIMA:** statistical time-series model fitted independently to each eligible asset.  "
                "\n**XGBoost:** machine-learning forecasting model evaluated on the same chronological test procedure.  "
                "\n**Evaluation:** predictions are tested against later known observations; lower MAE, RMSE and MAPE indicate better performance."
            )

elif page == "Data Lab":
    st.markdown(
        '<div class="terminal-head"><div class="live-line">ANALYST DATA LAB</div>'
        '<div class="terminal-title">Import and inspect data before model approval</div>'
        '<div class="terminal-sub">Validate structure · review missing values · explore prices interactively</div></div>',
        unsafe_allow_html=True,
    )
    st.info(
        "Preview mode only: an uploaded CSV is analysed within this dashboard session. "
        "It does not retrain the forecasting models or replace the stored project outputs."
    )
    uploaded_file = st.file_uploader(
        "Upload a cryptocurrency CSV",
        type=["csv"],
        help=(
            "Required: timestamp/date and price_usd/close/price. "
            "Optional: symbol, name and volume/vol_24h."
        ),
    )

    if uploaded_file is None:
        st.markdown(
            '<div class="easy-call"><div class="easy-label">EXPECTED STRUCTURE</div>'
            '<div class="easy-value">Timestamp + Price</div>'
            '<div class="metric-note">Symbol and volume are optional. Uploading does not change the deployed model.</div></div>',
            unsafe_allow_html=True,
        )
    else:
        try:
            uploaded_raw = pd.read_csv(uploaded_file, low_memory=False)
            ...
            if uploaded_raw.empty:
                st.error("The uploaded CSV contains column labels but no data rows.")
            else:
                uploaded_data, upload_details = prepare_uploaded_dataset(uploaded_raw)
                if upload_details["errors"]:
                    for upload_error in upload_details["errors"]:
                        st.error(upload_error)
                    st.caption(
                        "Detected columns: "
                        + ", ".join(str(column) for column in uploaded_raw.columns)
                    )
                elif uploaded_data.empty:
                    st.error("No usable rows remained after timestamp and price validation.")
                else:
                    raw_missing = int(uploaded_raw.isna().sum().sum())
                    date_start = uploaded_data["timestamp"].min()
                    date_end = uploaded_data["timestamp"].max()
                    asset_count = int(uploaded_data["symbol"].nunique())
                    st.markdown(
                        '<div class="metric-grid">'
                        + metric("Original rows", f"{len(uploaded_raw):,}", uploaded_file.name)
                        + metric("Usable rows", f"{len(uploaded_data):,}", "After timestamp and price checks")
                        + metric("Detected assets", f"{asset_count:,}", "Unique symbols")
                        + metric("Missing cells", f"{raw_missing:,}", "Before validation")
                        + '</div>',
                        unsafe_allow_html=True,
                    )
                    st.caption(
                        f"Validated period: {date_start:%d %b %Y} to {date_end:%d %b %Y} · "
                        f"invalid timestamps: {upload_details['invalid_timestamp_rows']:,} · "
                        f"invalid prices: {upload_details['invalid_price_rows']:,} · "
                        f"duplicate asset–timestamps removed: {upload_details['duplicate_rows']:,}."
                    )

                    uploaded_symbols = sorted(uploaded_data["symbol"].dropna().astype(str).unique())
                    uploaded_symbol = st.selectbox(
                        "Uploaded asset",
                        uploaded_symbols,
                        key="uploaded_asset_selector",
                    )
                    uploaded_asset = uploaded_data.loc[
                        uploaded_data["symbol"].eq(uploaded_symbol)
                    ].sort_values("timestamp").copy()

                    minimum_date = uploaded_asset["timestamp"].min().date()
                    maximum_date = uploaded_asset["timestamp"].max().date()
                    selected_dates = st.date_input(
                        "Visible date range",
                        value=(minimum_date, maximum_date),
                        min_value=minimum_date,
                        max_value=maximum_date,
                        key=f"uploaded_dates_{uploaded_symbol}",
                    )
                    if isinstance(selected_dates, (tuple, list)) and len(selected_dates) == 2:
                        selected_start, selected_end = selected_dates
                        uploaded_asset = uploaded_asset.loc[
                            uploaded_asset["timestamp"].dt.date.between(selected_start, selected_end)
                        ].copy()

                    if uploaded_asset.empty:
                        st.warning("No observations are available inside the selected date range.")
                    else:
                        uploaded_prices = uploaded_asset["price"].dropna()
                        uploaded_move = (
                            (uploaded_prices.iloc[-1] / uploaded_prices.iloc[0] - 1) * 100
                            if len(uploaded_prices) >= 2 and uploaded_prices.iloc[0] > 0
                            else np.nan
                        )
                        st.markdown(
                            '<div class="metric-grid">'
                            + metric("Latest price", money(uploaded_prices.iloc[-1]), "Last visible observation")
                            + metric("Period minimum", money(uploaded_prices.min()), "Lowest visible price")
                            + metric("Period maximum", money(uploaded_prices.max()), "Highest visible price")
                            + metric(
                                "Period move",
                                pct(uploaded_move),
                                "First-to-last visible price",
                                "up" if finite(uploaded_move) and uploaded_move >= 0 else "down",
                            )
                            + '</div>',
                            unsafe_allow_html=True,
                        )

                        upload_chart = go.Figure()
                        upload_chart.add_trace(
                            go.Scatter(
                                x=uploaded_asset["timestamp"],
                                y=uploaded_asset["price"],
                                mode="lines",
                                name="Observed price",
                                line=dict(color=ACCENT_COLOR, width=2.2),
                                hovertemplate=(
                                    "Date: %{x|%d %b %Y %H:%M}<br>"
                                    "Price: $%{y:,.6f}<extra></extra>"
                                ),
                            )
                        )
                        upload_chart.update_layout(
                            title=f"{uploaded_symbol} uploaded price history",
                            xaxis_title="Timestamp",
                            yaxis_title="Price",
                            dragmode="zoom",
                        )
                        st.plotly_chart(
                            style_chart(upload_chart, 500),
                            width="stretch",
                            theme=None,
                            config={"displaylogo": False, "scrollZoom": True},
                        )
                        if (
                            "volume" in uploaded_asset.columns
                            and uploaded_asset["volume"].notna().any()
                        ):
                            # Keep one rolling 24-hour volume observation per day.
                            # Do not sum because vol_24h is already a 24-hour value.
                            volume_daily = (
                                uploaded_asset[["timestamp", "volume"]]
                                .dropna(subset=["timestamp", "volume"])
                                .set_index("timestamp")
                                .resample("1D")
                                .last()
                                .dropna()
                            ...
                            volume_chart = go.Figure(
                                go.Bar(
                                    ...
                                    y=volume_daily["volume"],
                                    name="24H volume",
                                    marker=dict(
                                        color=ACCENT_COLOR,
                                        line=dict(color=ACCENT_COLOR, width=0.5),
                                    ),
                                    hovertemplate=(
                                        "Date: %{x|%d %b %Y}<br>"
                                        "24H volume: $%{y:,.0f}<extra></extra>"
                                    ),
                                )
                            )

                            volume_chart.update_layout(
                                title=f"{uploaded_symbol} daily 24-hour volume",
                                xaxis_title="Date",
                            ...
                            volume_chart.update_yaxes(
                                tickformat=".3s",
                                rangemode="tozero",
                            )

                            st.plotly_chart(
                                style_chart(volume_chart, 360),
                                width="stretch",
                                theme=None,
                                config={
                                    "displaylogo": False,
                    ...
                    missing_by_column = uploaded_raw.isna().sum().sort_values(ascending=False)
                    missing_by_column = missing_by_column.loc[missing_by_column.gt(0)]
                    if missing_by_column.empty:
                        st.success("No blank cells were detected in the uploaded CSV.")
                    else:
                        missing_chart = go.Figure(
                            go.Bar(
                                x=missing_by_column.values,
                                y=missing_by_column.index.astype(str),
                                orientation="h",
                                marker_color=SECONDARY_COLOR,
                                text=missing_by_column.values,
                                textposition="outside",
                                hovertemplate="%{y}: %{x:,} missing cells<extra></extra>",
                            )
                        )
                        missing_chart.update_layout(
                            title="Missing values in the uploaded CSV",
                            xaxis_title="Missing cells",
                            yaxis_title=None,
                        )
                        st.plotly_chart(
                            style_chart(missing_chart, max(300, 55 * len(missing_by_column))),
                            width="stretch",
                            theme=None,
                            config={"displayModeBar": False},
                        )

                    with st.expander("Preview validated rows"):
                        st.dataframe(uploaded_data.head(200), width="stretch", hide_index=True)
    ...
    if finite(future_move) and abs(float(future_move)) < .05:
        future_value = f"NEAR FLAT ({pct(future_move)})"
        future_note = "The stored model scenario shows no material directional change."
    elif finite(future_move) and future_move > 0:
        future_value = pct(future_move)
        future_note = "The stored model scenario finishes higher."
    elif finite(future_move):
        future_value = pct(future_move)
        future_note = "The stored model scenario finishes lower."
    else:
        future_value = "—"
        future_note = "Run the forecast engine to create a model scenario."
    st.markdown(f'<div class="terminal-head"><div class="live-line">MARKET OVERVIEW · {html.escape(str(symbol).upper())}</div><div class="terminal-title">What happened and what may happen</div><div class="terminal-sub">Observed market evidence is kept separate from the stored 30-day model scenario</div></div>', unsafe_allow_html=True)
    cards = st.columns(3)
    easy = [
        ("CURRENT MARKET · OBSERVED", f"{money(anchor_price)} · {pct(today_move)}", "Observed price is moving up over 24 hours." if finite(today_move) and today_move > 0 else "Observed price is moving down over 24 hours." if finite(today_move) else "Latest available observed market price."),
        ("PREVIOUS 30 DAYS · OBSERVED", pct(month_move), "The coin gained value over the observed period." if finite(month_move) and month_move > 0 else "The coin lost value over the observed period." if finite(month_move) else "A historical one-month comparison."),
        ("NEXT 30 DAYS · MODEL SCENARIO", future_value, future_note),
    ]
    ...
    line = go.Figure()
    line.add_trace(go.Scatter(x=history["timestamp"], y=history["close"], name="Observed history", line=dict(color=ACCENT_COLOR, width=2.2)))
    if not path.empty:
        line.add_trace(go.Scatter(x=path["date"], y=path["close"], name="Stored model scenario", line=dict(color="#53657a", width=3, dash="dash"), mode="lines+markers"))
    line.update_layout(title="Observed price history and stored 30-day model scenario", yaxis_title="USD")
    st.plotly_chart(style_chart(line, 520), width="stretch", theme=None, config={"displaylogo": False})
    action_tone = "up" if score >= 10 else "down" if score <= -10 else "warn"
    st.markdown(f'<div class="signal-core" style="min-height:220px"><div class="signal-content"><div class="signal-kicker">DECISION-SUPPORT SUMMARY</div><div class="signal-action {action_tone}">{html.escape(action)}</div><div class="signal-score">EVIDENCE {decision["evidence_strength"]:.0f}/100 · RISK {html.escape(str(decision["risk"]))}</div></div></div>', unsafe_allow_html=True)
    st.caption(
        "The summary combines observed technical evidence with a reduced-weight model scenario. "
        "It is a market-watch assessment, not a probability, financial recommendation or executed order."
    )

st.markdown('<div class="fine-print">Observed market data describes what happened. Model scenarios describe what may happen and are not guaranteed prices. This dashboard provides decision support, not personalised financial advice, and places no orders.</div>', unsafe_allow_html=True)
