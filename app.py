            ...
            if uploaded_raw.empty:
                ...
                if upload_details["errors"]:
                    ...
                    if uploaded_asset.empty:
                        ...
                        if "volume" in uploaded_asset and uploaded_asset["volume"].notna().any():
                            volume_chart = go.Figure(
                                go.Bar(
                                    x=uploaded_asset["timestamp"],
                                    y=uploaded_asset["volume"],
                                    name="Volume",
                                    marker_color=rgba(ACCENT_COLOR, .55),
                                    hovertemplate=(
                                        "Date: %{x|%d %b %Y %H:%M}<br>"
                            ...
                            volume_chart.update_layout(
                                title=f"{uploaded_symbol} uploaded volume",
                                xaxis_title="Timestamp",
                                yaxis_title="Volume",
                            )
                            st.plotly_chart(
                                style_chart(volume_chart, 330),
                                width="stretch",
                                theme=None,
                                config={"displayModeBar": False},
                            )
