    ...
    if not path.empty:
        ...
        if not horizon_row.empty and path["open"].iloc[0] > 0:
            ...
            horizon_multiplier = float(
                np.interp(
                    int(np.clip(decision_day, 1, 30)),
                    [1, 3, 7, 14, 30],
                    [1.00, .90, .75, .55, .30],
                )
            )
            effective_model_weight = model_weight * confidence_multiplier * horizon_multiplier
            add_factor(
                ...
                (
                    ...
                    f"horizon weight {horizon_multiplier:.0%}"
                    if finite(model_confidence)
                    else f"{model_move:+.2f}% · limited strength · horizon weight {horizon_multiplier:.0%}"
                ),
            )
...
st.sidebar.markdown('<div class="brand"><div class="brand-name">◈ Crypto<span>Forecast</span></div><div class="brand-sub">MARKET MONITORING · SCENARIOS · VALIDATION</div></div>', unsafe_allow_html=True)
page = st.sidebar.radio(
    "Workspace",
    ["Market Overview", "Live Market", "Decision Support", "Market Scanner", "Model Validation", "Data Lab"],
)
...
if page == "Live Market":
    ...
    st.caption(
        ...
        "The model contribution is reduced progressively at longer horizons because validated error rises with time."
    )
    ...
    st.markdown(
        '<div class="terminal-head"><div class="live-line">MODEL VALIDATION</div>'
        '<div class="terminal-title">How well did the forecasts generalise?</div>'
        '<div class="terminal-sub">Original performance compared with genuinely unseen assets · consistent 1D, 3D, 7D and 14D tests</div></div>',
    ...
    original_asset_count = int(comparison["symbol"].nunique()) if "symbol" in comparison else 0
    external_asset_count = int(external_comparison["symbol"].nunique()) if "symbol" in external_comparison else 0
    st.caption(
        f"Original evaluation: {original_asset_count} modelled assets · independent validation: "
        f"{external_asset_count} unseen assets. The unseen assets remain separate from model development."
    )
    cards = st.columns(3)
    for column, model in zip(cards, ["Naive", "ARIMA", "XGBoost"]):
        ...
        with column:
            st.markdown(
                metric(
                    f"{display_names[model]} · {status_label}",
    ...
    if independent_summary.empty:
        ...
        selected_model_labels = ["Naïve", "ARIMA"]
        ...
        selected_display["model"] = selected_display["model"].replace(display_names)
        ...
        fig = px.bar(
            ...
            barmode="group",
            text_auto=".2f",
            category_orders={"model": selected_model_labels},
        ...
        st.info(
            "Decision: retain Naïve as the validated benchmark and ARIMA as the selected statistical model. "
            "Both remain the top two on unseen assets, but both record higher error. This supports limited "
            "generalisation, not guaranteed accuracy."
        )
        ...
        if not horizon_frame.empty:
            horizon_frame["Model"] = horizon_frame["Model"].replace(display_names)
            fig = px.line(
                horizon_frame,
                x="Horizon",
                y="MAPE",
                ...
                markers=True,
                category_orders={"Model": selected_model_labels},
                title="Forecast error increases farther into the future",
            )
            fig.update_layout(xaxis_title="Days ahead", yaxis_title="MAPE (%) · lower is better")
            st.plotly_chart(
                style_chart(fig, 430),
                width="stretch",
                theme=None,
    ...
    st.info(
        ...
        "It does not retrain the forecasting models or replace the validated datasets."
    )
